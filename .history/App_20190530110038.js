...
export default class App extends Component<Props> {
  ...
  async momoHandleResponse(response){
    try{
      if (response && response.status == 0) {
        ...
        let message = response.message;
        //continue to submit momoToken,phonenumber to server
        this.setState({ description: "message: Get token success", processing: true });
      } else {
  ...
  async handlePress()
  {
    fetch('http://192.168.43.177:8000/api/savetoken', {
        method: 'POST',
        headers: {
      ...
      .then((responseJson) => {
   console.log("Author name at 0th index:  " + responseJson);
      })
      .catch((error) => {
        console.error(error);
      });
